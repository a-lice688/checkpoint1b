...
float celestialX = - 100; //starts off-screen left
float celestialY = 600; //starts below ground
//float skyTransition = 0;
boolean day = true;
color dayTop, dayBottom, nightTop, nightBottom, groundLeftColour, groundRightColour;
...
void animateCelestial() {
 ...
 celestialX += 2;
 float angle = map(celestialX, -100, width + 100, 0, PI); //had to look these up
 celestialY = 400 - 300 * sin(angle);
